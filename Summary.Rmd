...
```
```{r echo=FALSE,results="asis"}
  climate_stats = temperature_summary(climate, 0)
  ggplot(data = climate_stats, aes(y = num_events, x = site)) + geom_col()  +
  coord_flip()
  ggplot(data = climate_stats, aes(y = max_magnitude, x = site)) + geom_col()  +
  coord_flip()
  ggplot(data = climate_stats, aes(y = round(as.numeric(avg_magnitude)), x = site)) + geom_col()  +
  coord_flip()
  ggplot(data = climate_stats, aes(y = duration, x = site)) + geom_col()  +
  coord_flip()
  cat("\n")
